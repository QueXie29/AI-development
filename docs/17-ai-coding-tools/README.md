...
> **阅读提示：** 代码补全、仓库上下文、补丁验证、沙箱、评测和团队治理属于稳定考点；具体版本发布、定价和榜单只代表当时快照。面试前应核对官方资料，不建议背诵产品周报式题目。
...
### Q52: SWE-bench Pro 是什么？为什么它是 2025 年最重要的编码基准变化？

<a href="../../assets/illustrations/17-ai-coding-tools/q52-swebench-pro.webp"><img src="../../assets/illustrations/17-ai-coding-tools/q52-swebench-pro.webp" alt="AI编程工具专题第52题机制与工程取舍图解" width="100%"></a>

> 🧠 **图解记忆：** SWE-bench Pro 用人工专家审过的真实企业级 Issue 和隐私代码集，解决数据污染和短期任务问题。

<details>
<summary>💡 答案要点</summary>

**背景：** SWE-bench Verified 分数从 2024 年的 ~12% 飙升至 2026 年初的 80%+，但业界发现很多高分是靠"刷题"和数据污染达成的。OpenAI 甚至在 2026 年 2 月正式宣布停止报告 SWE-bench Verified 分数。

SWE-bench Pro = 专门针对这些问题的下一代基准测试：

| 特性 | SWE-bench Verified | SWE-bench Pro |
------|-------------------|---------------|
| **数据来源** | GitHub 公开仓库 Issue | 公开 + 隐私/商用仓库混合 |
| **任务难度** | 平均 12 行代码 / 单文件 | 平均 107 行代码 / 跨 4 个文件 |
| **人工质检** | ❌ | ✅ 每个任务由专业软件工程师审核描述、清理歧义测试 |
| **语言覆盖** | 仅 Python | 123 种编程语言 / 41 个活跃仓库 |
| **长度跨度** | 短周期修复 | 长周期复杂开发 |
| **数据污染风险** | 高（来自训练集的公开仓库） | 低（包含闭源仓库 + 严格隔离） |

**核心发现：**
- 最先进模型在 SWE-bench Pro 公共集上仅解决 <45% 问题
- 在专有/商业集上解决 <20% — 说明企业真实代码仍是 AI 的难点
- 相比原始 SWE-bench 的 12 行单文件，Pro 需要平均 107 行代码、修改 4 个文件、处理长期依赖关系

**为什么是重大突破：**
- 它首次区分了"刷过 SWE-bench 的模型"和"真正能完成复杂软件工程的模型"
- 1,865 个问题 × 41 个仓库 × 123 种语言 = 极其多样化和不可预知的场景
- 为招聘和选型提供了更可靠、防污染的衡量标准

**面试话术：**
> "SWE-bench Pro 的出现标志着 AI 编程评估进入第二阶段。早期模型靠刷题达到 80%+ 的 SWE-bench Verified 分数，但这反映的是对训练集中数据的记忆而非真正的软件工程能力。SWE-bench Pro 通过引入人工专家审核的任务、多文件修改、123 种语言和私有代码集，把通过率拉回不到 45% 的水平。这说明 AI 编程仍在起步阶段——特别是处理企业级复杂代码库的能力还有巨大提升空间。"

</details>

### Q53: SWE-EVO 是什么？它在软件演化场景中如何评测 Coding Agent？

<a href="../../assets/illustrations/17-ai-coding-tools/q53-swe-evo.webp"><img src="../../assets/illustrations/17-ai-coding-tools/q53-swe-evo.webp" alt="AI编程工具专题第53题机制与工程取舍图解" width="100%"></a>

> 🧠 **图解记忆：** SWE-EVO 让 Agent 面对持续演化的代码库，考验规划适应力而不是单次修 Bug。

<details>
<summary>💡 答案要点</summary>

**SWE-EVO 设计理念：** 传统基准测试一次给一个孤立 Issue，Agent 修复即结束。现实中开发者需要在不断变化的代码库中迭代维护。

SWE-EVO 模拟真实场景：

```
真实工作流 vs SWE-EVO 评测流
───────────────────────────────────────
真实：需求A → 实现 → 发布 → 需求B → 重构 → 发布B
       ↑_______________________________↓
         上下文在变，之前的改动影响新任务

SWE-EVO：从真实开源项目的版本历史中提取连续需求变更
Agent必须在已改动的代码基础上继续完成任务
```
...
| 维度 | 说明 |
------|------|
| **多步规划** | 需要理解一系列连续需求的依赖关系 |
| **适应性** | 当上游 API 或接口改变时调整实现 |
| **长期推理** | 跨多个提交保持架构一致性 |
| **回归控制** | 新功能不破坏已有行为 |

**数据来源：** 基于 Django、NumPy 等知名 Python 项目的版本化历史，提取真实的软件需求规格（SRS）。

**与 SWE-bench 的核心区别：**
- SWE-bench = 静态快照：给定当前状态和问题，一次性修复
- SWE-EVO = 动态演化：需求会按时间线逐步提出，Agent 必须在前序结果上继续工作

**面试话术：**
> "SWE-EVO 的价值在于它模拟了'真实项目不是修完一个 Bug 就结束了'这个常识。我们的工作中更多的是在已有的代码上添加功能、做重构、响应需求变更。SWE-EVO 用 Django 和 NumPy 的真实版本历史构建测试集，要求 Agent 在多次迭代后仍然保持代码质量。这比 SWE-bench 的单次修复更难，也更接近日常工作。"

</details>

### Q54: SlopCodeBench 是什么？为什么它揭示了 Coding Agent 在迭代任务中的代码退化问题？

<a href="../../assets/illustrations/17-ai-coding-tools/q54-slopcodbencq.webp"><img src="../../assets/illustrations/17-ai-coding-tools/q54-slopcodbencq.webp" alt="AI编程工具专题第54题机制与工程取舍图解" width="100%"></a>

> 🧠 **图解记忆：** Agent 每次都能通过检查点，但累积的代码质量在悄悄下降——这就是 SlopCodeBench 揭示的问题。
...
SlopCodeBench：Agent 反复扩展自己的解决方案
        ↓
    每轮都有新的需求和旧代码的影响
    关键：不仅看当前是否通过，还看整体代码质量趋势
```

**评测设置：**
- 36 个编程问题 / 196 个检查点
- Agent 每轮都在上一轮的结果上做增量修改
- 需求持续演变，需要持续的架构决策

**核心发现（反直觉）：**
1. **Checkpoint Passing ≠ 代码质量好** — Agent 通过了每个阶段的检查点，但整体代码质量在每轮迭代中缓慢退化
2. **代码膨胀** — 每一轮新增的功能都会增加代码复杂度，形成债务
3. **路径依赖性** — 早期的设计决策约束了后期的改进选项
4. **非收敛性** — 随着需求越来越多，系统越来越难以保持稳定

**工程启示：**
- 单纯提高单次生成准确率不够，需要加入代码质量监控和重构提醒
- 显式的质量指导可以减少约 1/3 的退化速度和膨胀幅度
- 长期运行的 Agent 工作流必须定期做代码审查和重构，不能只追加不改

**面试话术：**
> "SlopCodeBench 揭露了一个重要的工程现实：即使 Agent 每轮都通过了检查点，代码质量也可能在迭代中逐渐恶化。这是'技术债务'概念的自动化版本。在实际生产中，我们不能只追求'功能可用'，还需要定期做代码质量门禁和重构。优秀的团队会把 AI 生成的代码纳入同样的 Code Review 流程。"

</details>

### Q55: MCP Server 在生产环境部署的安全最佳实践有哪些？

<a href="../../assets/illustrations/17-ai-coding-tools/q55-mcp-security.webp"><img src="../../assets/illustrations/17-ai-coding-tools/q55-mcp-security.webp" alt="AI编程工具专题第55题机制与工程取舍图解" width="100%"></a>

> 🧠 **图解记忆：** MCP 连接数据库、文件系统甚至 Stripe 账户等于接入微服务——最小权限、只读优先、回滚能力缺一不可。

<details>
<summary>💡 答案要点</summary>

**MCP 安全模型：** MCP Server 本质上是一个微服务接口，AI 客户端通过标准化协议调用它操作你的资源。安全风险不在于协议本身，而在于你给它什么权限。

**生产环境十大安全原则：**

| # | 原则 | 具体措施 |
---|------|----------|
| 1 | **最小权限** | 按项目分 Key、限定目录范围，只暴露需要的工具 |
| 2 | **只读优先** | 先配置只读 Server（文档搜索、日志查看），验证稳定后再开放写入 |
| 3 | **TLS 加密** | SSE/HTTP Server 必须启用 HTTPS/TLS |
| 4 | **密钥管理** | 使用 Vault 或环境变量存储凭证，禁止硬编码在 mcp.json |
| 5 | **日志审计** | 记录每次 MCP 工具调用，追踪 AI 如何使用你的数据 |
| 6 | **回滚能力** | 对写入类操作必须有撤销方案（备份快照、事务机制） |
| 7 | **网络隔离** | 数据库 Server 运行在内网，不暴露到公网 |
| 8 | **超时控制** | 配置 result-size limits 防止大查询阻塞上下文窗口 |
| 9 | **权限分级** | 不同 MCP Server 用不同认证级别（如 ReadKey / AdminKey） |
| 10 | **渐进式集成** | 1-2 个最痛的服务开始，效果验证后再逐步增加 |

**GitHub MCP Server 最佳配置示例：**
...
```json
{
  "mcpServers": {
    "github": {
      ...
      "env": {
        "GITHUB_TOKEN": "${GITHUB_READ_ONLY_PAT}"  // 用只读 PAT
      },
      "headers": {
        "X-MCP-Readonly": "true"  // 强制只读模式
      }
    }
  }
}
...
**常见 MCP Server 类型及其安全关注点：**

| Server 类型 | 操作对象 | 最大风险等级 | 建议策略 |
|-------------|---------|-------------|---------|
| **filesystem** | 本地文件读写 | 🔴 高 | 限定项目根目录，禁止父目录访问 |
| **postgres/mongodb** | 数据库查询 | 🟡 中高 | 只读连接，禁用 DDL/DML |
| **git** | Git 操作 | 🟡 中 | 限定当前仓库，禁止 push 到 main |
| **github** | PR/Issue/CI | 🟡 中高 | 使用只读 PAT，限制 scope |
| **slack/email** | 消息/邮件 | 🟢 中低 | 审批制发送，禁止自动群发 |
| **stripe/payment** | 支付操作 | 🔴🔴 极高 | 生产环境禁用，仅沙箱测试 |

**面试话术：**
> "MCP Server 的生产安全本质上是零信任原则的应用。我把 MCP 服务器当成微服务对待——最小权限、TLS 加密、密钥分离、操作审计、只读优先。特别强调一点：任何涉及写入操作的 MCP Server 都必须有回滚能力。就像你说的'先只读、后写入'，这是一个经过验证的最佳实践路径。"

</details>

### Q56: SWE-rebench 是什么？为什么 OpenAI 停止报告 SWE-bench 分数？

<a href="../../assets/illustrations/17-ai-coding-tools/q56-swe-rebench-contam.webp"><img src="../../assets/illustrations/17-ai-coding-tools/q56-swe-rebench-contam.webp" alt="AI编程工具专题第56题机制与工程取舍图解" width="100%"></a>

> 🧠 **图解记忆：** SWE-rebench 去除了脚手架偏见和训练集污染，还原真实的模型能力排名。
...
```
SWE-bench Verified 分数攀升曲线（2024-2026）：
2024.03  SWE-agent: 12.47% ← 起点
2025.07  mini-SWE-agent: 65% ← 同一框架大幅跃升
2026.03  Claude Opus 4.5: 80.9% ← 行业标杆

⚠️ 但这些数字混杂了三个因素：
  1) 模型能力提升
  2) 脚手架优化（Prompt/Retrieval/Harness 特调）
  3) 数据污染（模型见过训练集中的 GitHub Issue）
```

SWE-rebench 通过统一脚手架来消除第 2 个因素：

| 对比项 | SWE-bench 官方榜单 | SWE-rebench |
--------|-------------------|-------------|
| **运行环境** | 各实验室用自己优化的 Agent Scaffold | 统一使用 mini-SWE-agent |
| **脚手架优势** | 大（优化后的 Prompt/检索大幅提升） | 消除（所有模型同一起跑线） |
| **数据污染** | 未校正 | 部分校正（标注已知污染实例） |
| **排名变化** | Claude > GPT > Gemini | 排序可能完全不同 |

**OpenAI 退出 SWE-bench 报告（2026.02）：**

OpenAI 正式声明两个原因：
1. **数据污染**：测试案例来自公开 GitHub 仓库，这些仓库的代码很可能已经进入训练集
2. **测试套件缺陷**：通过单元测试不等于修复正确——有些补丁只是"假装修复"而非真正解决问题

**SWE-rebench 的发现（2026.03）：**
- Claude Opus 4.6：65.3%
- GLM-5：62.8%
- DeepSeek V3.2：60.9%
- ⚠️ 这与 SWE-bench Verified 上的排名有明显差异，说明很多之前领先的优势来自脚手架优化而非模型能力

**工程意义：**
- SWE-bench Verified 适合展示"我的系统能做什么"（包括好的脚手架）
- SWE-rebench 更适合比较"模型本身的编码能力"（排除脚手架差异）
- 两者互补，不应该互相替代

**面试话术：**
> "SWE-rebench 的核心价值在于它回答了'去掉脚手架优势后，哪个模型更强'这个问题。OpenAI 退出 SWE-bench 报告是一个重要信号——当连最有动力刷高分的机构都觉得分数失去可信度时，说明整个评估体系到了需要升级的时刻。我认为在面试讨论中，同时引用两个榜单的数据才能全面展现对领域发展的理解。"
